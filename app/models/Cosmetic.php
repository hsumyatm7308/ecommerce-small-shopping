...
class Cosmetic
{
    ...
    public function items($offset, $limit)
    {
        ...
        $query = 'SELECT i.*, b.name AS brandname FROM items i INNER JOIN brands b ON b.id = i.brand_id WHERE i.category_id IN (6,7,8,9) AND 1=1';
        $bindparams = [];
    ...
    {
        $this->db->dbquery('SELECT * FROM categories WHERE id IN (6,7,8,9)');
        return $this->db->getmultidata();
    }
    ...
    public function countItems()
    {
        ...
        $query = 'SELECT COUNT(*) AS totalItems FROM items i WHERE i.category_id IN (6,7,8,9) AND 1 = 1';
        $bindParams = [];
