...
class Cosmetics extends Controller
{
    ...
    {
        $this->cosmeticmodal = $this->model('Cosmetic');
    ...
    public function index()
    {
        ...
        $totalitems = $this->cosmeticmodal->countItems();

        $totalPages = ceil($totalitems / $itemsperpage);


        $items = $this->cosmeticmodal->items($offset, $itemsperpage);
        $types = $this->cosmeticmodal->types();
        ...
        $userinfo = $this->cosmeticmodal->getuserinfo();
        ...
        $this->view('cosmetics/index', $data);
    }
    ...
    public function show($id)
    {
        ...
        $singledata = $this->cosmeticmodal->getsingleitem($id);
        $brand = $this->cosmeticmodal->getbrand($singledata['id']);
        $status = $this->cosmeticmodal->getstatus($singledata['status_id']);
        $authuser = $this->cosmeticmodal->getuserinfo();
        ...
        if (isset($_POST['addtocart'])) {
            if ($this->cardmodal->shopcardlist()) {
                $redirect_url = 'cosmetics/show/' . $this->curitemidmodal->getitemid() . '?message=added';
                redirect($redirect_url);
            } else {
                $redirect_url = 'cosmetics/show/' . $this->curitemidmodal->getitemid() . '?message=already_added';
                redirect($redirect_url);
            }
            ...
            if ($this->cardmodal->shopcardlist()) {
                $redirect_url = 'cosmetics/show/' . $single_recom_itemid;
                redirect($redirect_url);
            }
        }
