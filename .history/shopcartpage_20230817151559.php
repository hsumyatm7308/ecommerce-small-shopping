                        ...
                        <div class="w-full h-10 bg-gray-200 flex justify-between items-center">
                            ...
                            <h1 class="text-black mr-7">Subtotal: <span class="text-indigo-500 ml-2 totalmount">
                                    <input type="text" id="subtotal"
                                        class="w-20 p-1 focus:outline-none totalprice" value="$<?php echo $totalamount; ?>"
                                        readonly>
                                </span>
                            </h1>
                            <script>
                                  const checksubtotal =document.getElementById('subtotal');
                                  totalamount = 0; // Reset totalamount
    // Loop through local storage data to calculate the new totalamount
    for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        if (key.startsWith("price-")) {
            totalamount += parseFloat(localStorage.getItem(key));
        }
    }

    // Update the UI with the new subtotal
    checksubtotal.value = "$" + totalamount.toFixed(2);                            </script>
                        </div>
