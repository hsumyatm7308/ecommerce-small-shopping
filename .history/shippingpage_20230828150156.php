...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    if (isset($_POST['submit'])) {
        ...
        $contact = $conn->prepare("UPDATE email SET email=:email FROM customerinfo WHERE temporary_id = :temp");
        $contact->bindParam('email', $emailvalue);
        $contact->bindParam('temp', $rowtemp['id']);
        $contact->execute();

        $address = $conn->prepare("UPDATE address SET address=:address FROM shippingaddress WHERE temporaryid = :temp");
