                        ...
                        <div class="w-full h-10 bg-gray-200 flex justify-between items-center">
                            ...
                            try {
                                ...
                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    $subtotal = $row['quantity'] * $row['perfumeprice']; // Calculate subtotal for the item
                                    $totalAmount += $subtotal; // Add subtotal to the total amount
                            
                                    // Display product details and subtotal
                                    echo "<p>{$row['product_name']} - Quantity: {$row['quantity']} - Subtotal: {$subtotal}</p>";
                                }
