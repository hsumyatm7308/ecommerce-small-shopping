...
try {
    ...
    // Fetch rows for both 'Women' and 'Unisex' categories
    $womenUnisexStmt = $conn->prepare("SELECT id, perfume_name, brand_name, category_name, price, imgurl, mili FROM perfume WHERE category_name IN ('Women', 'Unisex')");
    $womenUnisexStmt->execute(); // Execute the query to fetch data

    // Fetch rows for 'Unisex' category only if 'type' parameter is 'on'
    if (isset($_GET['type']) && $_GET['type'] === 'on') {
        ...
        // Combine the results of 'Women' and 'Unisex' into a single array
        $womenUnisexResults = array_merge($womenUnisexStmt->fetchAll(PDO::FETCH_ASSOC), $unisexStmt->fetchAll(PDO::FETCH_ASSOC));
    } else {
        // If 'type' parameter is not 'on', use only 'Women' category results
        $womenUnisexResults = $womenUnisexStmt->fetchAll(PDO::FETCH_ASSOC);
    }
