...
// Get the value of the 'letters' query parameter from the current URL
const existingLetters = getParameterByName('letters', currentURL);

// Split the existing letters into an array (if any)
const lettersArray = existingLetters ? existingLetters.split(',') : [];

// Check if the clicked letter is already in the array
if (!lettersArray.includes(letter)) {
  // If not, add it to the array
  lettersArray.push(letter);
}

// Create a new URL with the updated letters
const newURL = hasQuery
  ? `${currentURL}&letters=${lettersArray.join('&letters=')}`
  : `${currentURL}?letters=${lettersArray.join('&letters=')}`;
