    ...
    <header class="navbgmarker ">
        ...
        <nav class="w-screen h-auto  flex items-center grid grid-cols-4 p-3">
            ...
            <div class="flex justify-start items-center">
                <ul class="flex justify-start items-center text-[15px] uppercase">
                    <?php
                    $currentPage = $_GET['page'] ?? ''; // Get the 'page' query parameter or set to an empty string if not present
                    $currentMenPage = $_GET['menpage'] ?? ''; // Get the 'menpage' query parameter or set to an empty string if not present
                    $currentWomenPage = $_GET['womenpage'] ?? ''; // Get the 'menpage' query parameter or set to an empty string if not present
                    
                    // Check if on the 'All' page and add the class accordingly
                    if (empty($currentPage) || $currentPage === '1') {
                        echo '<li class="mr-5 border border-stone-300 px-2 py-2 rounded-full"><a href="index.php?page=1">All</a></li>';
                    } else {
                        echo '<li class="mr-5"><a href="index.php?page=1">All</a></li>';
                    }

                    // Check if on the 'Men' page and add the class accordingly
                    if (!empty($currentMenPage) && $currentMenPage === '1') {
                        echo '<li class="mr-5 border border-stone-300 px-2 py-2 rounded-full"><a href="menfrg.php?menpage=1">Men</a></li>';
                    } else {
                        echo '<li class="mr-5"><a href="menfrg.php?menpage=1">Men</a></li>';
                    }

                    if (!empty($currentMenPage) && $currentMenPage === '1') {
                        echo '<li class="mr-5 border border-stone-300 px-2 py-2 rounded-full"><a href="womenfrg.php?womenpage=1">Women</a></li>';
                    } else {
                        echo '<li class="mr-5"><a href="menfrg.php?menpage=1">Men</a></li>';
                    }
                    ?>
                    <li class="mr-5"><a href=""></a></li>
                </ul>
            </div>
