        ...
        try {
            ...
            if ($name === '' || $email === '' || $address === '') {
                ...
                ctntoshipbtn.addEventListener("click", function(event) {
                    var hasValue = false; // Assume no fields have a value initially
                    for (var i = 0; i < inputval.length; i++) {
                        if (inputval[i].value.trim() !== "") {
                            hasValue = true; // At least one field has a value
                            inputval[i].classList.remove("border", "border-red-500");
                        } else {
                            inputval[i].classList.add("border", "border-red-500");
                        }
                    }
                    
                    // Check if any input field has a value; if they do, don't prevent the default action.
                    if (!hasValue) {
