...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST["quantity"]) && isset($_POST["perfumeName"]) && isset($_POST["brandName"])) {
        $quantity = $_POST["quantity"];
        $id = $_GET['items'];
        $perfumeName = $_POST["perfumeName"];
        $brandName = $_POST["brandName"];

        echo $quantity, $id,$perfumeName;
        ...
        try {
            ...
            while ($row = $cartstmt->fetch()) {
                // Access columns of the fetched row using $row['column_name']
