    ...
    if (isset($_POST['ctntoship'])) {
        ...
        try {
            ...
            if ( $email === '') {
                ...
                $select = $conn->prepare('SELECT temporary_id FROM customerinfo WHERE email = :email');
                $select->bindParam(":email", $user_identifier);
                $select->execute();
                
                $tempRow = $select->fetch();
                $temp_customer_id = $tempRow['temporary_id'];
