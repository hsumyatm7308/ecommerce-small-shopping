...
<body class="">
    ...
    <header class="navbgmarker ">
        ...
        <nav class="w-screen h-auto  flex items-center grid grid-cols-4 p-3">
            ...
            <div class="flex justify-start items-center">
                <ul class="flex justify-start items-center text-[15px] uppercase">
                    <!-- Add a unique id to each li element to help with JavaScript -->
                    <li id="allLi" class="mr-5 border border-stone-300 px-2 py-2 rounded-full"><a
                            href="index.php?page=1">All</a></li>
                    <li id="menLi" class="mr-5"><a href="menfrg.php?menpage=1">Men</a></li>
                    <li class="mr-5"><a href="womenfrg.php?womenpage=1">Women</a></li>
                </ul>
            </div>
            ...
            <script>
                ...
                // Function to add the border to the active li element
                function setActiveLiElement() {
                    const allLiElements = document.querySelectorAll('li');
                    const menLiElement = document.getElementById('menLi');
                    const allLiElement = document.getElementById('allLi');

                    // Remove the border from all li elements
                    allLiElements.forEach(li => li.classList.remove('border-stone-200'));

                    // Check if on the 'Men' page and set the border on the 'Men' li
                    if (currentPageURL.includes('menfrg.php')) {
                        menLiElement.classList.add('border-stone-200');
                    }
                    // Check if on the 'All' page and set the border on the 'All' li
                    else if (currentPageURL.includes('index.php')) {
                        allLiElement.classList.add('border-stone-200');
                    }
                }

                // Call the function on page load to set the initial active li element
                window.onload = setActiveLiElement;
            </script>
