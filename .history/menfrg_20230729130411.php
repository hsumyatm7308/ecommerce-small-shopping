    ...
    function handleLetterClick(letter) {
        ...
        for (const letter of updatedLetters) {
            ...
            innerSpan.addEventListener('click', function () {
                ...
                // Remove the clicked letter from the DOM (the innerSpan)
                this.remove();

                // Get the current clickedLetters value from sessionStorage
                let clickedLetters = sessionStorage.getItem('clickedLetters') || '';

                // Split the clickedLetters string into an array of characters
                const lettersArray = clickedLetters.split('');

                // Find the index of the clicked letter in the array
                const indexToRemove = lettersArray.indexOf(clickedLetter);

                // If the clicked letter is found in the array, remove it
                if (indexToRemove !== -1) {
                    lettersArray.splice(indexToRemove, 1);

                    // Join the array back into a string and save the updated clickedLetters back to sessionStorage
                    clickedLetters = lettersArray.join('');
                    sessionStorage.setItem('clickedLetters', clickedLetters);
