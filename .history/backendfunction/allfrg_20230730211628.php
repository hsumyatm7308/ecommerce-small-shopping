...
try {
    ...
    if (isset($_GET['startprice']) && isset($_GET['endprice'])) {
        ...
        if ($startprice !== false && $endprice !== false && $startprice >= 0 && $endprice >= $startprice) {
            ...
            $priceFilter = "AND price BETWEEN :startprice AND :endprice";
        }
    }

    $stmt = $conn->prepare("SELECT id, perfume_name, brand_name, category_name, price, imgurl, mili FROM perfume WHERE category_name IN ('Men','Unisex','Women') $priceFilter LIMIT $startfrom, $recperpage");

    // Bind the price filter parameters if they are provided
