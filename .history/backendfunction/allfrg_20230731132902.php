...
try {
    ...
    if (isset($_GET['startprice']) && isset($_GET['endprice'])) {
        ...
        // Pagination for price-based search
        $recperpage = 12;
        $totalrec = $pricestmt->rowCount();
        $totalpages = ceil($totalrec / $recperpage);

        $price = isset($_GET['price']) ? (int) $_GET['price'] : 1;
        $startfromprice = max(0, ($price - 1) * $recperpage);
        $pricestmt->bindParam(':startfromprice', $startfromprice, PDO::PARAM_INT);
        $pricestmt->bindParam(':recperpage', $recperpage, PDO::PARAM_INT);
