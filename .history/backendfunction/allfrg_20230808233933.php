...
if (isset($_POST['asc'])) {
    ...
    // $sortOption="review";
    try {
        $stmt = $conn->prepare("SELECT perfume_id, COUNT(*) AS total_reviews FROM reviewtable GROUP BY perfume_id ORDER BY total_reviews DESC LIMIT 1");
        $stmt->execute();

        if ($row = $stmt->fetch()) {
            $mostReviewedItemId = $row['perfume_id'];
            $totalReviews = $row['total_reviews'];

            // Now you have the most reviewed item's ID and its total review count
        } else {
