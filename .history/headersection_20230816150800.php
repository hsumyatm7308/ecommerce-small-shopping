...
$(document).ready(function () {
    ...
    if ($_SERVER['REQUEST_METHOD'] === "POST") {
        if (isset($_POST['action']) && $_POST['action'] === "count") {
            $itemcount = $_POST['itemscount'];
            echo "var itemcount = " . json_encode($itemcount) . ";"; // Pass the item count to JavaScript
        }
    }
    ?>
    
    $(".countcart").text(itemcount); // Update the item count on the page
});
</script>
Please note the changes I made:

I've defined the itemcount variable using PHP and then encoded it using json_encode to ensure it's properly converted for JavaScript usage.
The JavaScript code inside $(document).ready() ensures that the code runs after the page's DOM is fully loaded, which is necessary to access and update elements on the page.
Please make sure that the AJAX request in your original code is sending the correct data to itemspage.php to receive the updated item count in response.
