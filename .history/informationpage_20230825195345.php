    ...
    if (isset($_POST['ctntoship'])) {
        ...
        try {
            ...
            if ($email === '') {
                ...
                try {
                    // Establish a database connection (assuming $conn is already set up)
                
                    // Prepare and execute a SELECT query to count rows in the 'customerinfo' table
                    $select = $conn->prepare("SELECT COUNT(*) FROM customerinfo");
                    $select->execute();
                    $rowCount = $select->fetchColumn();
                
                    if ($rowCount > 0) {
                        // There are existing records, so we'll update them
                        ...
                        // No existing records, so we'll insert a new one
