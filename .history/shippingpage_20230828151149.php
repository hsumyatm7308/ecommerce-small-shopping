...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    if (isset($_POST['submit'])) {
        ...
        $updateemail = $conn->prepare("");
        $updateemail->bindParam('email', $emailvalue);
        $updateemail->bindParam('temp', $rowtemp['id']);
        $updateemail->execute();
        
        $updateaddr = $conn->prepare("");
        $updateaddr->bindParam('address', $addressvalue);
        $updateaddr->bindParam('temp', $rowtemp['id']);
        $updateaddr->execute();
        ...
        $address = $conn->prepare("UPDATE shippingaddress SET email = :email WHERE temporaryid = :tempid");
        $address->bindParam('temp', $temp['id']);
        $address->execute();
