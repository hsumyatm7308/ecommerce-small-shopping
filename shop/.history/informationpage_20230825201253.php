            ...
            if ($email === '') {
                ...
                try {
                    ...
                    if ($rowCount > 0) {
                        ...
                        $temp_customer_id_numeric = intval($temp_customer_id);

                        $updateStmt = $conn->prepare('UPDATE customerinfo SET email = :email WHERE temporary_id = :tempid');
                        $updateStmt->bindParam(":tempid", $temp_customer_id_numeric);

                        $updateStmt->bindParam(":email", $email);
                        $updateStmt->execute();


                        echo "Updated $rowCount records with temporary ID:" . $temp_customer_id_numeric;
