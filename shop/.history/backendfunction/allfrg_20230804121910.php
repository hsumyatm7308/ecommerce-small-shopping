...
try {
    ...
    if (isset($_GET['search'])) {
        ...
        // Make sure to adjust column names according to your database structure
        $searchstmt = $conn->prepare("SELECT id, perfume_name, brand_name, category_name, price, imgurl, mili 
                                      FROM perfume 
                                      WHERE (perfume_name LIKE :searchTerm OR brand_name LIKE :searchTerm) 
                                      AND category_name IN ('Men', 'Unisex', 'Women') 
                                      ORDER BY $sortOption");
        
        // Bind the search term to the placeholder in the query
