...
if (isset($_POST['asc'])) {
    ...
    $sortOption = "review_count DESC"; // Sort by review count in descending order
}

// Fetch the review count for each perfume using a separate query
$reviewCounts = [];
$reviewCountQuery = "SELECT perfume_id, COUNT(id) AS review_count FROM reviewtable GROUP BY perfume_id";
$reviewCountStmt = $conn->prepare($reviewCountQuery);
$reviewCountStmt->execute();
while ($row = $reviewCountStmt->fetch(PDO::FETCH_ASSOC)) {
    $reviewCounts[$row['perfume_id']] = $row['review_count'];
}
...
try {
    ...
    if (isset($_GET['search'])) {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $row['review_count'] = isset($reviewCounts[$row['id']]) ? $reviewCounts[$row['id']] : 0;
            $results[] = $row;
        }

        // Sort the results based on the selected sorting option
        usort($results, function ($a, $b) use ($sortOption) {
            if ($sortOption === "review_count DESC") {
...
<div class="grid grid-cols-3">
    ...
    if (isset($_GET['search'])) {
        ...
        // Loop through the sorted results and display them
        foreach ($results as $row) {
            echo '<div class="w-[95%] bg-gray-100 p-3 m-2 rounded flex justify-center items-center flex-col hover:opacity-80">';
            $binary_data = $row['imgurl'];
            $base64_image = base64_encode($binary_data);
