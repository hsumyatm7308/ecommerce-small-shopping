...
try {
    ...
    if (isset($_GET['search'])) {
        ...
        WHERE (p.perfume_name LIKE :searchTerm OR p.brand_name LIKE :searchTerm)
        AND p.category_name IN ('Men', 'Unisex', 'Women')
        ORDER BY $sortOption
        ");
...
<div class="grid grid-cols-3">
    ...
    if (isset($_GET['search'])) {
        ...
        while ($row = $searchstmt->fetch()) {
            echo '<div class="w-[95%]  p-3 m-2 rounded flex justify-center items-center flex-col hover:opacity-80">';
            ...
            echo '<a href="./itemspage.php?items=' . $row["id"] . '"class="w-64 self-start mt-3">' . $row['perfume_name'] . ' by ' . $row['brand_name'] . ' EDT 3.3 OZ ' . $row['mili'] . ' spray for ' . $row['category_name'] . '</a>';
            echo '<span class="self-start font-semibold mt-2">$ ' . $row['price'] . '</span>';
            echo '</div>';
        }
        ...
        while ($row = $pricestmt->fetch()) {
            echo '<div class="w-[95%]   p-4 m-2 rounded flex justify-center items-center flex-col hover:opacity-80">';
            $binary_data = $row['imgurl'];
            $base64_image = base64_encode($binary_data);
            echo '<img src="data:image/jpeg;base64,' . $base64_image . '" alt="Image" style="max-width: 200px;" >';
            echo '<a href="./itemspage.php?items=' . $row["id"] . ' "class="w-64 self-start mt-3">' . $row['perfume_name'] . ' by ' . $row['brand_name'] . ' EDT 3.3 OZ ' . $row['mili'] . ' spray for ' . $row['category_name'] . '</a>';
            echo '<span class="self-start mt-2 font-semibold hover:opacity-80 cursor-pointer py-2">$ ' . $row['price'] . '</span>';
            echo '</div>';
        }
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            echo '<img src="data:image/jpeg;base64,' . $base64_image . '" alt="Image" style="max-width: 200px;" >';
            echo '<a href="./itemspage.php?items=' . $row["id"] . '" class="w-64 self-start mt-3">' . $row['perfume_name'] . ' by ' . $row['brand_name'] . ' EDT 3.3 OZ ' . $row['mili'] . ' spray for ' . $row['category_name'] . '</a>';
            echo '<a href="" class="self-start mt-2  hover:opacity-80  font-semibold cursor-pointer py-2">$ ' . $row['price'] . '</a>';
            echo '</div>';
