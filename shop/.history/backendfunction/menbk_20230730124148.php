...
if (isset($_GET['letters'])) {
    ...
    // Retrieve the existing 'letters' cookie and unserialize it as an array
    $selectedLetters = isset($_COOKIE['letters']) ? unserialize($_COOKIE['letters']) : array();

    // Add the current letter to the array if it's not already present
    if (!in_array($letter, $selectedLetters)) {
        $selectedLetters[] = $letter;
    }

    // Store the updated array in the 'letters' cookie (serialize it before storing)
    setcookie('letters', serialize($selectedLetters), time() + (86400 * 30), '/');
} else {
    // If 'letters' parameter is not provided, check if the cookie is already set
    $selectedLetters = isset($_COOKIE['letters']) ? unserialize($_COOKIE['letters']) : array();
}
...
try {
    ...
    // Modify the query to use the $selectedLetters array
    if (!empty($selectedLetters)) {
        $placeholders = str_repeat('?,', count($selectedLetters) - 1) . '?';
        $query = "SELECT id, perfume_name, brand_name, category_name, price, imgurl, mili FROM perfume WHERE category_name IN ('Men','Unisex') AND fsletter IN ($placeholders) ORDER BY fsletter ASC";
        $menstmt = $conn->prepare($query);
        $menstmt->execute($selectedLetters);
    } else {
        // If no letters are selected, retrieve all perfumes
        $menstmt = $conn->prepare("SELECT id, perfume_name, brand_name, category_name, price, imgurl, mili FROM perfume WHERE category_name IN ('Men','Unisex')");
        $menstmt->execute();
    }
} catch (Exception $e) {
