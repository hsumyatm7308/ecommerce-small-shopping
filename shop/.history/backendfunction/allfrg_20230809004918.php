...
try {
    ...
    if (isset($_GET['search'])) {
        ...
        // $stmt = $conn->prepare("SELECT id, perfume_name, brand_name, category_name, price, imgurl,mili FROM perfume WHERE category_name IN ('Men','Unisex','Women') ORDER BY review");
        $stmt = $conn->prepare("
        SELECT p.id, p.perfume_name, p.brand_name, p.category_name, p.price, p.imgurl, p.mili, COUNT(r.id) AS review_count
        FROM perfume AS p
        LEFT JOIN reviewtable AS r ON p.id = r.perfume_id
        WHERE p.category_name IN ('Men', 'Unisex', 'Women')
        GROUP BY p.id, p.perfume_name, p.brand_name, p.category_name, p.price, p.imgurl, p.mili
...
try {
    ...
    if (isset($_GET['price']) && isset($_GET['price'])) {
        ...
        // Define the valid sorting options to prevent SQL injection
        $validSortOptions = ['id', 'perfume_name', 'brand_name', 'category_name', 'price', 'imgurl', 'mili'];
        $defaultSortOption = 'id'; // Default sorting option if the provided one is invalid

        // Define pagination variables
        $startfrom = isset($_GET['startfrom']) ? intval($_GET['startfrom']) : 0;
        $recperpage = 10; // Number of records per page

        // Validate and sanitize the input sort option
        $sortOption = isset($_GET['sort']) && in_array($_GET['sort'], $validSortOptions)
            ? $_GET['sort']
            : $defaultSortOption;
