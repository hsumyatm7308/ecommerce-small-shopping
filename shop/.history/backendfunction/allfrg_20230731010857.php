...
try {
    ...
    if (isset($_GET['startfrom']) && isset($_GET['endform'])) {
        ...
        $stmt = $conn->prepare("SELECT id, perfume_name, brand_name, category_name, price, imgurl, mili FROM perfume LIMIT $startfromprice, $recperpage");
        $stmt->execute();
...
if (isset($_GET['startprice']) && isset($_GET['endprice'])) {
    // If startprice and endprice parameters are present, hide pagination
    // You can optionally display a message to inform the user that pagination is disabled
