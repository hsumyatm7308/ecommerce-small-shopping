...
try {
    ...
    if (isset($_GET['startprice']) && isset($_GET['endprice'])) {
        ...
        $menstmt = $conn->prepare("SELECT p.*, COALESCE(r.review_count, 0) AS review_count FROM perfume p LEFT JOIN (SELECT perfume_id, COUNT(userreview) AS review_count FROM reviewtable GROUP BY perfume_id) r ON p.id = r.perfume_id WHERE p.category_name IN ('Men','Unisex') ORDER BY $sortOption");
        ...
        if (isset($_GET['type']) && $_GET['type'] === 'on') {
            ...
            $menstmt = $conn->prepare("SELECT p.*, COALESCE(r.review_count, 0) AS review_count FROM perfume p LEFT JOIN (SELECT perfume_id, COUNT(userreview) AS review_count FROM reviewtable GROUP BY perfume_id) r ON p.id = r.perfume_id WHERE p.category_name IN ('Unisex') ORDER BY $sortOption");
            $menstmt->execute();
        ...
        if (isset($_GET['type']) && $_GET['type'] === 'mon') {
            ...
            $menstmt = $conn->prepare("SELECT p.*, COALESCE(r.review_count, 0) AS review_count FROM perfume p LEFT JOIN (SELECT perfume_id, COUNT(userreview) AS review_count FROM reviewtable GROUP BY perfume_id) r ON p.id = r.perfume_id WHERE category_name = 'Men' ORDER BY $sortOption");
...
try {
    ...
    if (isset($_GET['price']) && isset($_GET['price'])) {
        ...
        WHERE p.price BETWEEN :startprice AND :endprice AND p.category_name IN ('Men','Unisex') 
        ORDER BY $sortOption
    ...
    ");
        ...
        WHERE p.category_name IN ('Men','Unisex')
        ORDER BY $sortOption
    ...
    ");
        ...
        if (isset($_GET['type']) && $_GET['type'] === 'on') {
            ...
            WHERE p.category_name IN ('Unisex')
            ORDER BY $sortOption
            LIMIT $startfrom, $recperpage
        ");
            $unisexstmt->execute();
...
if (isset($_GET['letters'])) {
    ...
    try {
        ...
        WHERE p.category_name IN ('Men','Unisex') AND p.fsletter = :letter
        ORDER BY $sortOption
        LIMIT $startfrom, $recperpage
    ");
