...
try {
    ...
    if (isset($_GET['price']) && isset($_GET['price'])) {
        ...
        $startfromprice = max(0, ($pricePage - 1) * $recperpage);

        echo "totalpages".$startfrom;
        // Fetch the price filtered items for the current page
        $stmt = $conn->prepare("SELECT id, perfume_name, brand_name, category_name, price, imgurl, mili FROM perfume WHERE price BETWEEN :startprice AND :endprice LIMIT $startfromprice, $recperpage");
