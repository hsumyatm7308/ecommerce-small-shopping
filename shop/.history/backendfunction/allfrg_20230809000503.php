...
try {
    ...
    if (isset($_GET['search'])) {
        ...
        $stmt = $conn->prepare("
            SELECT p.id, p.perfume_name, p.brand_name, p.category_name, p.price, p.imgurl, p.mili,
                   (SELECT COUNT(r.id) FROM reviewtable r WHERE r.perfume_id = p.id) AS review_count
            FROM perfume p
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            // Output the perfume data including review_count
            echo "ID: {$row['id']}, Name: {$row['perfume_name']}, Brand: {$row['brand_name']}, Category: {$row['category_name']}, Price: {$row['price']}, Review Count: {$row['review_count']}<br>";
        }
