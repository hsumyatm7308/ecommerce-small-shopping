...
try {
    ...
    if (isset($_GET['startprice']) && isset($_GET['endprice'])) {
        ...
        $url = "http://localhost/perumcommance/ecommerce/index.php?startprice=$startprice&endprice=$endprice&price=1";
        // Perform any necessary actions, e.g., redirecting to the constructed URL
        header("Location: " . $url);
        exit;   
...
try {
    ...
    if (isset($_GET['price']) && isset($_GET['startprice']) && isset($_GET['endprice'])) {
        $startprice = $_GET['startprice'];
        $endprice = $_GET['endprice'];
