...
try {
    ...
    if (isset($_GET['startprice']) && $_GET['endprice']) {
        ...
        $stmt = $conn->prepare("SELECT id, perfume_name, brand_name, category_name, price, imgurl, mili FROM perfume   WHERE price BETWEEN $startprice AND $endprice  LIMIT $startfrom, $recperpage");
        $stmt->execute();
        // echo "Category: Unisex
    }
...
<div class="flex justify-center items-center mt-10 p-10">
    ...
    if (isset($_GET['type']) && $_GET['type'] === 'on') {
        ...
        if (isset($_GET['pricequery'])) {
            ...
            echo '<a href="?startprice=1" class="text-blue-500 border px-2 py-1 ml-10">Next</a>';
