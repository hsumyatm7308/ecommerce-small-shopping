...
try {
    ...
    if (isset($_GET['search'])) {
        ...
        $query = "
            SELECT perfume.id, perfume.perfume_name, perfume.brand_name, perfume.category_name, perfume.price, perfume.imgurl, perfume.mili,
                   (SELECT COUNT(r.id) FROM reviewtable r WHERE perfume.id = r.perfume_id) AS review_count
            FROM perfume perfume
            WHERE perfume.category_name IN ('Men', 'Unisex', 'Women')
            ORDER BY $sortOption
        ";
