...
try {
    ...
    if (isset($_GET['updateprice']) && $_GET['updateprice'] === 'p') {

        $startprice = $_GET['startprice'];
        $endprice = $_GET['endprice'];


        $stmt = $conn->prepare("SELECT id, perfume_name, brand_name, category_name, price, imgurl, mili FROM perfume WHERE price BETWEEN :startprice AND :endprice category_name = 'Women' LIMIT $startfrom, $recperpage");
        $stmt->execute();
        // echo "Category: Unisex
    }
