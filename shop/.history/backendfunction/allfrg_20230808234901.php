...
if (isset($_POST['asc'])) {
    ...
    // Using a subquery to calculate review count for each perfume
    $sortOption = "review_count DESC"; // Sort by review count in descending order
    
    $show = $conn->prepare("
        SELECT rt.*, COUNT(r.id) AS review_count
        FROM reviewtable rt
        LEFT JOIN reviewtable r ON rt.perfume_id = r.perfume_id
        GROUP BY rt.perfume_id
        ORDER BY $sortOption
    ");
    $show->execute();

    while ($row = $show->fetch(PDO::FETCH_ASSOC)) {
        // Output the review data here
    }
}
