...
<div class="flex justify-center items-center mt-10 p-10">
    ...
    if (isset($_GET['type']) && $_GET['type'] === 'on') {
        ...
        if (isset($_GET['unisexquery']) && $_GET['unisexquery'] > 1) {
            $prevUnisexPage = $_GET['unisexquery'] - 1;
            echo '<a href="?type=on&unisexquery=' . $prevUnisexPage . '&letters=' . urlencode($_GET['letters']) . '" class="text-red-500 border px-2 py-1 mr-10">Prev</a>';
        }

        for ($x = 1; $x <= $totalpages; $x++) {
            echo '<a href="?type=on&unisexquery=' . $x . '&letters=' . urlencode($_GET['letters']) . '" class="w-6 h-7 border p-1 m-1 flex justify-center items-center active">' . $x . '</a>';
        }
