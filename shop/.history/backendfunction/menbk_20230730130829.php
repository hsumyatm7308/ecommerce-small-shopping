...
// Check and set the cookie first
if (isset($_GET['letters']) && is_array($_GET['letters'])) {
    $letters = $_GET['letters'];

    // Now $letters is an array containing the letters from the URL.
    $shownLetters = array();

    // You can loop through the array to perform actions for each letter.
    foreach ($letters as $letter) {
        // echo $letter . "afjdsa;";
        $shownLetters[] = $letter;

        try {
            global $conn; // Make sure $conn is defined and contains a valid database connection.
            ...
            echo $letter . "lkjas";
            print_r($shownLetters);
        } catch (Exception $e) {
            echo "Error Found: " . $e->getMessage();
        }
    }

    // Serialize the $shownLetters array into a string before setting the cookie
    $serializedLetters = serialize($shownLetters);

    // Set the cookie with the serialized array
    setcookie('myLettersCookie', $serializedLetters, time() + 3600, '/'); // Cookie expiration set to 1 hour (3600 seconds)
}

// Rest of your PHP code and HTML content below
?>
