...
try {
    ...
    if (isset($_GET['startprice']) && isset($_GET['endprice'])) {
        ...
        echo "helo".$price."totoal".$totalpages;

        $stmt = $conn->prepare("SELECT id, perfume_name, brand_name, category_name, price, imgurl, mili FROM perfume WHERE price BETWEEN :startprice AND :endprice AND category_name IN ('Men','Unisex','Women')  LIMIT $startfromprice, $recperpage");
        $stmt->execute();
...
<div class="flex justify-center items-center mt-10 p-10">
    ...
    if (isset($_GET['startprice']) && isset($_GET['endprice'])) {
        ...
        // echo '<span>Pagination is disabled for filtered results.</span>';
        if (isset($_GET['price']) && $_GET['price'] > 1) {
            $prevMenPage = $_GET['page'] - 1;
            echo '<a href="?price=' . $prevMenPage . '" class="text-red-500 border px-2 py-1 mr-10">Prev</a>';
        }

        for ($x = 1; $x <= $totalpages; $x++) {
            echo '<a href="?price=' . $x . '" class="w-6 h-7 border p-1 m-1 flex justify-center items-center active">' . $x . '</a>';
        }
        ...
        if (isset($_GET['price'])) {
            if ($_GET['price'] >= $totalpages) {
                ...
                if (isset($_GET['price']) && is_numeric($_GET['price'])) {
                    ...
                    echo '<a href="?price=' . $nextPage . '" class="text-red-500 border px-2 py-1 ml-10">Next</a>';
                }
            }
        } else {
            echo '<a href="?price=1" class="text-blue-500 border px-2 py-1 ml-10">Next</a>';
        }
