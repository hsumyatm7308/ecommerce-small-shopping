...
try {
    ...
    if (isset($_GET['startprice']) && $_GET['endprice']) {
       $startprice = $_GET['startprice'];
    $endprice = $_GET['endprice'];

    // Sanitize the input to prevent SQL injection
    $startprice = filter_var($startprice, FILTER_VALIDATE_FLOAT);
    $endprice = filter_var($endprice, FILTER_VALIDATE_FLOAT);

    if ($startprice !== false && $endprice !== false && $startprice >= 0 && $endprice >= $startprice) {
        // Apply the price filter to the query
        $stmt = $conn->prepare("SELECT id, perfume_name, brand_name, category_name, price, imgurl, mili FROM perfume WHERE price BETWEEN :startprice AND :endprice AND category_name IN ('Men','Unisex','Women') LIMIT $startfrom, $recperpage");
...
try {
    ...
    if (isset($_GET['startprice']) && isset($_GET['endprice'])) {
        ...
        if ($startprice !== false && $endprice !== false && $startprice >= 0 && $endprice >= $startprice) {
            // Apply the price filter to the query
            $priceStmt = $conn->prepare("SELECT id, perfume_name, brand_name, category_name, price, imgurl, mili FROM perfume WHERE price BETWEEN :startprice AND :endprice AND category_name IN ('Men','Unisex','Women') LIMIT $startfrom, $recperpage");
            $priceStmt->bindParam(':startprice', $startprice, PDO::PARAM_INT);
            $priceStmt->bindParam(':endprice', $endprice, PDO::PARAM_INT);
            $priceStmt->execute();

            // Calculate the total pages for price filter pagination
...
<div class="flex justify-center items-center mt-10 p-10">
    ...
    } elseif ($priceStmt !== null) {
        // Display the pagination links for price filtering
        if (isset($_GET['pricequery']) && $_GET['pricequery'] > 1) {
            $prevMenPage = $_GET['pricequery'] - 1;
            echo '<a href="?startprice=' . $startprice . '&endprice=' . $endprice . '&pricequery=' . $prevMenPage . '" class="text-red-500 border px-2 py-1 mr-10">Prev</a>';
        }

        for ($x = 1; $x <= $totalpages; $x++) {
            echo '<a href="?startprice=' . $startprice . '&endprice=' . $endprice . '&pricequery=' . $x . '" class="w-6 h-7 border p-1 m-1 flex justify-center items-center active">' . $x . '</a>';
        }

        if (isset($_GET['pricequery'])) {
            if ($_GET['pricequery'] >= $totalpages) {
            } else {
                $nextPage = $_GET['pricequery'] + 1;
                echo '<a href="?startprice=' . $startprice . '&endprice=' . $endprice . '&pricequery=' . $nextPage . '" class="text-red-500 border px-2 py-1 ml-10">Next</a>';
            }
        } else {
            echo '<a href="?startprice=' . $startprice . '&endprice=' . $endprice . '&pricequery=1" class="text-blue-500 border px-2 py-1 ml-10">Next</a>';
        }
    } else {
