...
<?php
$currentPage = 1; // Replace this with the actual current page number, which you can get from query parameters or session data
?>
...
<?php while ($row = $stmt->fetch()): ?>
    ...
    if (!in_array($row['fsletter'], $shownLetters)) {
        ...
        <li class="w-7 h-7 flex justify-center items-center m-1 letter-link <?php echo $backgroundClass; ?>">
            <a href="index.php?page=<?php echo $currentPage; ?>&letters=<?php echo urlencode($letter); ?>"><?php echo $letter; ?></a>
        </li>
