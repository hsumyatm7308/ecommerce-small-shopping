                                    ...
                                    <div class="grid grid-cols-4 border-b border-b-solid border-b-gray-300 px-3 py-2">
                                        ...
                                        <div class="flex justify-center items-center">
                                            ...
                                            <input type="submit" name="submit"
                                                class="font-semibold text-red-500 changeinput hidden" value="Update">
...
if (isset($_POST['submit'])) {
    ...
    // Retrieve the temporary ID from the "tempidtable"
    $seletemp = $conn->prepare("SELECT id FROM tempidtable WHERE tempid = :temp");
    $seletemp->bindParam(":temp", $temp_customer_id);
    $seletemp->execute();
    ...
    // Update the "customerinfo" table with the new email
    $contact = $conn->prepare("UPDATE customerinfo SET email = :email WHERE temporary_id = :tempid");
    $contact->bindParam(':email', $emailvalue); // Corrected parameter name

    $contact->bindParam(':tempid', $temp['id']); // Corrected parameter name
    $contact->execute();

    // Update the "shippingaddress" table with the new address
    $address = $conn->prepare("UPDATE shippingaddress SET address = :address WHERE temporaryid = :tempid");
    $address->bindParam(':address', $addressvalue); // Corrected parameter name

    $address->bindParam(':tempid', $temp['id']); // Corrected parameter name
    $address->execute();
}
