...
<?php while ($row = $stmt->fetch()): ?>
    ...
    <div id="<?= $row['perfume_id'] ?>" class="cart-item grid grid-cols-6 justify-center items-center border-b mb-4">
        ...
        <script type="text/javascript">
            ...
            const savedquantity = localStorage.getItem(`qty-<?php echo $row['perfume_id'] ?>`);
            const savedprice = localStorage.getItem(`price-<?php echo $row['perfume_id'] ?>`);

            if (savedquantity !== null) {
                qtyinput<?= $row['perfume_id'] ?>.value = savedquantity;
            }
            if (savedprice !== null) {
                totalpriceinput<?= $row['perfume_id'] ?>.value = savedprice;
            }

            increasebtn<?= $row['perfume_id'] ?>.addEventListener('click', function () {
                qtyinput<?= $row['perfume_id'] ?>.value++;
                updateTotalPrice<?= $row['perfume_id'] ?>();
                savetolocalstorag<?= $row['perfume_id'] ?>e();
            });
            ...
            function savetolocalstorage<?= $row['perfume_id'] ?>() {
                localStorage.setItem(`qty-<?php echo $row['perfume_id'] ?>`, qtyinput<?= $row['perfume_id'] ?>.value);
                localStorage.setItem(`qty-<?php echo $row['perfume_id'] ?>`, totalpriceinput<?= $row['perfume_id'] ?>.value);
            }
        </script>
