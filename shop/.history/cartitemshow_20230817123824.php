...
<?php while ($row = $stmt->fetch()): ?>
    ...
    <div id="<?= $row['perfume_id'] ?>" class="cart-item grid grid-cols-6 justify-center items-center border-b mb-4">
            ...
            <div class="flex justify-center items-center">
                <div class="flex justify-center items-center">
                    <button type="submit" name="remove" id="remove-<?= $row['perfume_id'] ?>"
                        class="px-3 py-1 bg-gray-100 remove-item" data-item-id="<?= $row['perfume_id'] ?>">Remove</button>
                </div>
            </div>
        </form>
        ...
        <script>
            ...
            console.log(parseInt(eachqty) + "eachqty");
            console.log((qtyinput<?= $row['perfume_id'] ?>.value * price<?= $row['perfume_id'] ?>).toFixed(2) * eachqty + "subtotal");
